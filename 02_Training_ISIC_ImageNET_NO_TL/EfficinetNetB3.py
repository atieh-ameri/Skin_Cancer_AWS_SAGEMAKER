...
prefix = 'Data_tensorflow/data'  # Path to your main folder
...
# %%
# s3_path = 's3://data-set-aameri/Data_tensorflow/isic_2024_metadata.csv'
...
# Phase 1 training - freeze all layers
...
# Load the best weights from Phase 1 FIRST
...
def load_model_safe(model_path):
    ...
    try:
        # Define a simple f1B_score function
        def f1B_score(y_true, y_pred):
            # Simple implementation that should satisfy the model loading
            from tensorflow.keras import backend as K
            precision = K.precision(y_true, y_pred)
            recall = K.recall(y_true, y_pred)
            beta = 1.0  # Default value for standard F1
